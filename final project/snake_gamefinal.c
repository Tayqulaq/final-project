...
int main() {
    ...
    SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO); // Initialize SDL2 video and audio subsystems
    ...
    while (running) {
        ...
        if (hit_wall(&snake) || hit_self(&snake)) {
            ...
            Mix_PlayChannel(-1, gameOverSound, 0); // Play game over sound
            break;
        }
